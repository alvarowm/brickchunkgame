...
pub fn sbc_00001010 (cpu: &mut CPU, ram: &mut [u8; 256]) {
    ...
    cpu.acc.set_bit_range(0..7, (resultado as u8).bit_range(0..7));

    cpu.carry_flag = resultado >= 0;
...
pub fn check_interrupts(cpu: &mut CPU, rom: [u8; 4096], mutex: &Arc<Mutex<Vec<KeyEvent>>>) {
    ...
    if cpu.ei && mutex.lock().unwrap().len() > 0 && !(instruction.bit(0) &&
                ...
                instruction.bit(2) &&
                instruction.bit(3)) {
                let key = mutex.lock().unwrap().pop().unwrap().code;

                match  key{
                    //Start
                    crossterm::event::KeyCode::Char('s')=> {cpu.ps0 = true; set_stack_cf(cpu); cpu.pc = 8},
                    //on/off
                    crossterm::event::KeyCode::Char('o')=> {cpu.ps2 = true; set_stack_cf(cpu); cpu.pc = 8},
                    //Mute
                    crossterm::event::KeyCode::Char('m')=> {cpu.ps1 = true; set_stack_cf(cpu); cpu.pc = 8},
                    //Mute/Left
                    crossterm::event::KeyCode::Left =>{cpu.pp3 = true; set_stack_cf(cpu); cpu.pc = 8},
                    //Select/Down
                    crossterm::event::KeyCode::Down=>{cpu.pp1 = true; set_stack_cf(cpu); cpu.pc = 8},
                    //Mute/Right
                    crossterm::event::KeyCode::Right =>{cpu.pp2 = true; set_stack_cf(cpu); cpu.pc = 8},
                    //Rotate
                    crossterm::event::KeyCode::Char('r')=>{cpu.pp0 = true; set_stack_cf(cpu); cpu.pc = 8},
                    _ => {}
                }
            }
}
